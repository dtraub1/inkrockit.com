...
if ($postnukeIntegration == "true") {
	$this->pathImg = "modules/PhpCollab/themes";
} else {
	$this->pathImg = "../themes";
}
}
...
if ($postnukeIntegration == "true") {
	...
	$address = str_replace(".php?","&amp;",$address);
	$address = "index.php?module=PhpCollab&amp;func=main&amp;file=$address";
}
...
//	echo "<th nowrap class=\"$sortingStyles[$i]\"><a href=\"javascript:document.".$this->form."Form.sor_cible.value='$this->sortingRef';document.".$this->form."Form.sor_champs.value='$sortingFields[$i]';document.".$this->form."Form.sor_ordre.value='$sortingOrders[$i]';document.".$this->form."Form.submit();\" onMouseOver=\"javascript:window.status='".$strings["sort_by"]." ".addslashes($labels[$i])."'; return true;\" onMouseOut=\"javascript:window.status=''; return true\">".$labels[$i]."</a>$sortingArrows[$i]</th>\n";
	echo "<th nowrap class=\"main\"><a href=\"javascript:document.".$this->form."Form.sor_cible.value='$this->sortingRef';document.".$this->form."Form.sor_champs.value='$sortingFields[$i]';document.".$this->form."Form.sor_ordre.value='$sortingOrders[$i]';document.".$this->form."Form.submit();\" onMouseOver=\"javascript:window.status='".$strings["sort_by"]." ".addslashes($labels[$i])."'; return true;\" onMouseOut=\"javascript:window.status=''; return true\">".$labels[$i]."</a>$sortingArrows[$i]</th>\n";
} else {
if ($sortingOff[1] == "ASC") {
...
echo "<td width=\"30\" class=\"commandBtn\"><a href=\"javascript:var b = MM_getButtonWithName(document.".$this->form."Form, '".$this->form."$num'); if (b) b.click();\" onMouseOver=\"var over = MM_getButtonWithName(document.".$this->form."Form, '".$this->form."$num'); if (over) over.over(); return true; \" onMouseOut=\"var out = MM_getButtonWithName(document.".$this->form."Form, '".$this->form."$num'); if (out) out.out(); return true; \"><img width=\"23\" height=\"23\" border=\"0\" name=\"".$this->form."$num\" src=\"$this->pathImg/$this->theme/btn_".$type."_norm.gif\" alt=\"$text\"></a></td>\n";
}
...
if ($postnukeIntegration == "true") {
	...
	$link = "index.php?module=PhpCollab&amp;func=main&amp;file=$link";
}
echo "document.".$this->form."Form.buttons[document.".$this->form."Form.buttons.length] = new MMCommandButton('".$this->form."$num',document.".$this->form."Form,'".$link."&".session_name()."=".session_id()."','$this->pathImg/$this->theme/btn_".$type."_norm.gif','$this->pathImg/$this->theme/btn_".$type."_over.gif','$this->pathImg/$this->theme/btn_".$type."_down.gif','$this->pathImg/$this->theme/btn_".$type."_dim.gif',$options,'','".$text."',false,'');\n";
}
...
global $postnukeIntegration;
	if ($type == "in") {
		if ($postnukeIntegration == "true") {
			...
			return "<a href=\"index.php?module=PhpCollab&amp;func=main&amp;file=$url&amp;".session_name()."=".session_id()."\">$label</a>";
		} else {
